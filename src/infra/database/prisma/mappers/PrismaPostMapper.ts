...
export class PrismaPostMapper{
    static toPrisma(post: PostSchema): PostRaw {
        const data = new Post({ title: post.title, content: post.content, authorId: post.authorId, uppoints: post.uppoints, downpoints: post.downpoints, father_post_id: post.father_post_id })
        return {
            id: data.get_id,
    ...
    static toDomain(posts: PostRaw[]): Post[] {
        return posts.map(post => Post.reconstitute({
            title: post.title,
